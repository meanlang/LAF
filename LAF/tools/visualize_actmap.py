...
import deep_person_reid_master.torchreid as torchreid
from deep_person_reid_master.torchreid.utils import (
...
def output_feature(j, outputs, width, height, img_np):
    am = outputs[j, ...].numpy()
...
def visactmap(
    ...
    for target in list(test_loader.keys()):
        ...
        actmap_gl_dir = osp.join(save_dir, 'actmap_glo' + target)
        mkdir_if_missing(actmap_gl_dir)
        ...
        for batch_idx, data in enumerate(data_loader):
            ...
            try:
                outputs, outputs_global = model(imgs, return_featuremaps=True)
            except TypeError:
                raise TypeError(
            ...
            outputs_global = (outputs_global ** 2).sum(1)
            b, h, w = outputs_global.size()
            outputs_global = outputs_global.view(b, h * w)
            outputs_global = F.normalize(outputs_global, p=2, dim=1)
            outputs_global = outputs_global.view(b, h, w)


            if use_gpu:
                imgs, outputs = imgs.cpu(), outputs.cpu()
                imgs, outputs_global = imgs.cpu(), outputs_global.cpu()
            ...
            for j in range(outputs.size(0)):
                ...
                overlapped, am = output_feature(j, outputs, width, height, img_np)

                overlapped_gl, am_gl = output_feature(j, outputs_global, width, height, img_np)
                ...
                grid_img_gl = 255 * np.ones(
                    (height, 3 * width + 2 * GRID_SPACING, 3), dtype=np.uint8
                )
                grid_img_gl[:, :width, :] = img_np[:, :, ::-1]
                grid_img_gl[:,
                width + GRID_SPACING:2 * width + GRID_SPACING, :] = am_gl
                grid_img_gl[:, 2 * width + 2 * GRID_SPACING:, :] = overlapped_gl


                cv2.imwrite(osp.join(actmap_dir, imname + '.jpg'), grid_img)
                cv2.imwrite(osp.join(actmap_gl_dir, imname + '.jpg'), grid_img_gl)
