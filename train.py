...
from deep_person_reid_master.tools.visualize_actmap import visactmap
...
def get_criterion(num_classes: int, use_gpu: bool, args):
    ...
    if args.criterion == 'htri':
        ...
        criterion_t = TripletLoss(num_classes, vars(args), use_gpu)

        from torchreid.losses.cross_entropy_loss import CrossEntropyLoss
        criterion_c = CrossEntropyLoss(num_classes, use_gpu=use_gpu, label_smooth=args.label_smooth)

        return (criterion_t, criterion_c)
...
def main():
    ...
    if args.evaluate:
        ...
        for name in args.target_names:
            ...
            if args.visualize_actmap:
                save_dir = osp.join(args.save_dir, 'ranked_actmap', name)
                print(save_dir)
                visactmap(
                    model, testloader_dict, save_dir, args.width, args.height, use_gpu)
        return
...
def train(epoch, model, criterion, regularizer, optimizer, trainloader, use_gpu, fixbase=False):
    ...
    for batch_idx, (imgs, pids, _, _) in enumerate(trainloader):
        ...
        outputs = model(imgs)
        loss_trip = criterion[0](outputs, pids)
        loss_cross = criterion[1](outputs, pids)
        loss = 0.1 * loss_trip + loss_cross
...
def test(model, queryloader, galleryloader, use_gpu, ranks=[1, 5, 10, 20], return_distmat=False, return_feature=False):
